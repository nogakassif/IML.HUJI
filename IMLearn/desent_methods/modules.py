...
class L1(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        output: ndarray of shape (1,)
            Value of function at point self.weights
        """
        return np.sum(np.absolute(self.weights))
...
class LogisticModule(BaseModule):
    ...
    def compute_output(self, X: np.ndarray, y: np.ndarray,
        ...
        """
        m = X.shape[0]
        l = np.log(1 + np.exp(X @ self.weights))
        return (-1 / m) * np.sum(y * (X @ self.weights) - l)
    ...
    def compute_jacobian(self, X: np.ndarray, y: np.ndarray,
        ...
        """
        m = X.shape[0]
        exp = np.exp(self.weights @ X.T)
        return -(1 / m) * (y @ X - X.T @ (exp / (1 + exp)))
...
class RegularizedModule(BaseModule):
    ...
    def compute_output(self, **kwargs) -> np.ndarray:
        ...
        return self.fidelity_module_.compute_output(
            **kwargs) + self.lam_ * self.regularization_module_.compute_output(
            **kwargs)
    ...
    def compute_jacobian(self, **kwargs) -> np.ndarray:
        ...
        """
        j_fidelty = self.fidelity_module_.compute_jacobian(**kwargs)
        j_regularization = self.regularization_module_.compute_jacobian(
            **kwargs)

        if self.include_intercept_:
            j_regularization = np.insert(j_regularization, 0, 0, axis=0)

        return j_fidelty + j_regularization * self.lam_
