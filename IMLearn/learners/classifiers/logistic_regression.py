...
class LogisticRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        if self.penalty_ == "none":
            ...
            f = RegularizedModule(LogisticModule(), L1(), self.lam_, weights,self.include_intercept_)
        elif self.penalty_ == "l2":
            f = RegularizedModule(LogisticModule(), L2(), self.lam_, weights,self.include_intercept_)
    ...
    def _predict(self, X: np.ndarray) -> np.ndarray:
        ...
        return np.where(sigmoid > self.alpha_, 1, 0)
